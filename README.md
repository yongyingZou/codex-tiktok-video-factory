...
- 真人尽量少露脸；优先手部、身体局部、背影和商品演示，确认是AI人物时可以露脸。
- 不确定的商品事实不编造。
- 不承诺爆款或收益。
...
- `crop`：字幕靠近边缘时重新构图，在不损伤商品和手部动作的前提下裁掉；
- `replace`：只有区域较小且稳定时，完整覆盖原字幕并在相同区域显示同步的新口播字幕；
- `reject`：无法干净覆盖或会遮挡商品演示时，舍弃该镜头。

优先级是“干净镜头 → 自然裁切 → 语义合适的原字幕 → 小范围替换 → 舍弃”。系统不会
为了保住一个镜头不断扩大遮罩；大色块、明显模糊带、残留字形或遮挡商品都会触发重新
选镜头。具体剪辑计划格式见 `examples/edit-plan.example.json`。
...
→ 标记动作阶段、准确切入/切出点、前后镜头关系和支持的口播语义
→ 设计局部变速、动态关键词、动作音效、环境声和BGM避让
→ 优先选择无真人正脸的商品、手部、身体局部和场景镜头
→ 检查素材覆盖率、镜头复用、重复结尾和仅换顺序的伪变体
→ 已有成片只作为排重对象；重新从完整镜头库选片，禁止从旧 edit-plan 复制候选镜头
→ 渲染前强制检查相同/裁短时间段、重复开头结尾、购买理由和历史画面重合率
→ 生成 edit-plan JSON
→ Remotion执行动态构图与声音设计，FFmpeg完成底层媒体处理和编码
→ 技术质检、逐镜头视觉质检、完整视听质检和批量差异检查
```

新增视频的 edit-plan 还必须声明 `novelty.selection_basis=shot_library`、独立购买理由和
相对历史版本的差异。排重失败会在编码前停止；如果剩余素材无法支撑新的销售逻辑，系统应
报告素材不足，而不是仅换口播、音乐、顺序或裁切点生成“伪新视频”。

### 流程门禁：不再“分析失败也继续生成”

每个商品使用 `analysis/v1/workflow-state.json` 保存当前状态和已经锁定的选择。剪辑计划必须
声明 `production_stage`：

- `baseline`：商品、全素材分析、镜头库和方向确认通过后，只生成一条基准片；
- `batch`：只有基准片经过完整画面和声音审核并标记为 `approved` 后，才能生成其他方向。

原音频转写失败、视频或图片未全部检查、镜头库未完成或方向未确认时，渲染命令会拒绝执行，
而不是生成文件后再用技术QA掩盖问题。状态模板见 `examples/workflow-state.example.json`。

商品分析不会从文件夹辅助名称直接推断品类，也不会把水洗、颜色、数量等辅助信息自动
提升为独立视频方向。系统先建立“商品本质品类—核心需求—解决机制—使用场景—用户顾虑
—主卖点—辅助卖点—规格信息”的商品模型，再用全部视频的画面、口播和硬字幕验证、补充
或质疑它。

剪辑也不是给拼接结果定时添加转场。每个镜头必须说明为何从这一帧进入、为何在这一帧
离开、与前镜头是什么关系、支持哪句口播，以及是否需要变速、文字或动作音效。特效只在
动作、比较、意义转折或场景变化需要时使用；普通硬切往往比无意义的炫技更有效。
...
- 口播与视频描述分别创作：描述不能直接复制口播再追加一句行动引导；
- 每个话题标签都提供“日文标签—中文含义”对照；
- 每条视频单独保存发布记录，新增视频时从全部记录重建汇总Markdown，不覆盖历史条目；
